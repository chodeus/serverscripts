#!/bin/bash
# mover-tuning-start.sh v1.3.6 by BZ — https://github.com/TRaSH-Guides/Guides
...
trap '' PIPE  # Move button closes stdout mid-run; SIGPIPE must not kill us
...
# ================================
log() {
    # `|| true`: stdout may be a dead pipe under the plugin's Move button (PHP passthru)
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] $*" 2>/dev/null || true
}
    ...
    if "$venv_python" -m pip --version >/dev/null 2>&1; then
        ...
        if "$venv_python" -c "import qbittorrentapi" 2>/dev/null; then
            ...
            if [[ "$supports_dry_run" == true ]]; then
                ...
                if grep -q "Would install" <<<"$qbt_dry_out"; then
                    ...
                    set_ownership "$VENV_PATH"
                    log "✓ qbittorrent-api is $("$venv_python" -m pip show qbittorrent-api 2>/dev/null | awk '/Version:/ {print $2}')"
                else
...
process_qbit_instance() {
    ...
    if $python_cmd "$MOVER_SCRIPT" \
        ...
        --days_from "$DAYS_FROM" \
        --days_to "$DAYS_TO" \
        "${ca_bundle_args[@]}" >"$log_file" 2>&1; then
        log "✓ Successfully processed $name"
        { grep -hE "INFO - Pausing \[" "$log_file" | tail -1 | sed "s/^/    /"; } 2>/dev/null || true
        notify "$name" "Paused @ $(date +%H:%M:%S)"
        return 0
    else
        log "✗ Failed to process $name (details: $log_file)"
        return 1
    fi
}
